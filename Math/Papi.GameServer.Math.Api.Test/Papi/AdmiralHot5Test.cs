...
namespace Papi.GameServer.Math.Api.Test.Papi
{
    [TestClass]
    public sealed class AdmiralHot5Test : BaseTestClass
    {
        [TestMethod]
        [DataRow(Games.AdmiralHot5, 5)]
        [DataRow(Games.BonusBells, 10)]
        ...
        [TestMethod]
        [DataRow(Games.AdmiralHot5, 5, 5, 96.5, 20000, 2000000)]
        ...
        [TestMethod]
        [DataRow(Games.BonusBells, 10, 10, 96.25, 500000, 2000000)]
        public async Task GameBonusBells_ShouldHave_Rtp(Games game, int lines, int bet, double expectedRtp, int iterationsMin, int iterationMax)
        {
            //Arrange
            double totalBet = 0, totalWin = 0;
            var iterationCount = 0;
            ...
            while (iterationCount < iterationMax && !condition)
            {
                var rtpCalculation = await CalculateRtpForGamesWithFreeSpinsFeature(game, lines, bet, iterationsMin, totalBet, totalWin);
                iterationCount += iterationsMin;
                rtp = rtpCalculation.Rtp;
                totalBet = rtpCalculation.TotalBet;
                totalWin = rtpCalculation.TotalWin;
