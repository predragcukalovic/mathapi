...
namespace CombinationExtras.ConversionData.ByteArrayConversion
{
    class GameWildWestConversion
    {
            ...
            ICombination combination)
        {
            ...
            Array.Copy(combination.GratisGamesValues, 0, data, k, combination.GratisGamesValues.Length);
            k += 5;

            Array.Copy(combination.GratisGamesPositions, 0, data, k, combination.GratisGamesPositions.Length);
            k += 5;

            var emptyArray = Enumerable.Repeat((byte)255, 5).ToArray();
            Array.Copy(emptyArray, 0, data, k, 5);
            k += 5;
