...
{
    public class GameChristmasPresentsConversion
    {
        public static SlotDataResV3 ToSlotDataResV3(ICombination combination)
        {
            ...
            var thr = new List<CoordinateV3>();
            for (var i = 0; i < 4; i++)
            {
                var pos = (combination.WinFor2 >> (8 * i)) & 0x000000FF;
                if (pos > 0)
                {
                    var wld = new CoordinateV3
                    {
                        reel = pos % 5,
                        row = pos / 5 - 1
                    };
        ...
        {
            var matrixArray = new[,] { { 2, 2, 2, 2, 2 }, { 6, 6, 6, 6, 6 }, { 5, 5, 5, 5, 5 }, { 4, 4, 4, 4, 4 }, { 3, 3, 3, 3, 3 } };
