...
{
    public class GameSimplySevensConversion
    {
        public static SlotDataResV3 ToSlotDataResV3(ICombination combination)
        {
            ...
            for (var i = 0; i < 5; i++)
            {
                nearlyMissed[i, 0] = combination.Matrix[i, 0] == 10 ? 100 : combination.Matrix[i, 0];
                nearlyMissed[i, 1] = combination.Matrix[i, 4] == 10 ? 100 : combination.Matrix[i, 4];

                for (var j = 1; j < 4; j++)
                {
                    matrix[i, j - 1] = combination.Matrix[i, j] == 10 ? 100 : combination.Matrix[i, j];
                }
            }
        ...
        {
            var matrixArray = new[,] { { 7, 7, 7, 7, 7 }, { 6, 6, 6, 6, 6 }, { 5, 5, 5, 5, 5 }, { 4, 4, 4, 4, 4 }, { 8, 8, 8, 8, 8 } };
