...
{
    public class CombinationSuperLucky : Combination
    {
        public void MatrixToCombinationSuperLucky(MatrixSuperLucky matrix, int numberOfLines, int bet, bool gratisGame, ref byte[] addArray)
        {
            WinFor2 = bet;
            GratisGame = false;
            ...
            if (gratisGame)
            {
                ...
                for (var i = 0; i < 15; i++)
                {
                    if (addArray[i] == 0)
                    {
                        if (SoftwareRng.Next() < MatrixCloverCash.GratisLockProbs[elemNum - 6])
                        {
                            ...
                            var lockwin = MatrixCloverCash.GetWinByIndex(index, table) / 2;
                ...
                if (elemNum == 15)
                {
                    TotalWin += MatrixCloverCash.GRAND_JACKPOT * bet / 2;
                    lockWins.Add(new LineInfo { Id = 253, WinningElement = 11, Win = MatrixCloverCash.GRAND_JACKPOT * bet / 2, WinningPosition = new byte[] { 255, 255, 255, 255, 255 } });
                }
                ...
                if (!GratisGame)
                {
                    for (var i = 0; i < 15; i++)
                    {
                        if (addArray[i] > 0)
                        {
                            TotalWin += MatrixCloverCash.GetWinByIndex(addArray[i] - 1, table) * bet / 2;
                        }
                    }
                }
                return;
            ...
            CreateLinesInformations(matrix, numberOfLines, bet, 1, 0, MatrixSuperLucky.WinForWildsSuperLucky, GlobalData.GameLineExtra);
            ...
            if (lockit >= 6)
            {
                ...
                {
                    TotalWin += MatrixCloverCash.GRAND_JACKPOT * bet / 2;
                    lockWins.Add(new LineInfo { Id = 252, WinningElement = 11, Win = MatrixCloverCash.GRAND_JACKPOT * bet / 2, WinningPosition = new byte[] { 255, 255, 255, 255, 255 } });
                }
                else
                {
                    GratisGame = true;
